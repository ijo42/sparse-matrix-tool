...
void inmatrix::load(QString filename, QWidget *widget) {
    ...
    QFutureWatcher<std::pair<SparseDoubleLinkedMatrix*, bool>> *watcher = new QFutureWatcher<std::pair<SparseDoubleLinkedMatrix*, bool>>(widget);
    connect(watcher, &QFutureWatcher<std::pair<SparseDoubleLinkedMatrix*, bool>>::finished, widget, [widget, watcher, filename, progress]() {
        progress->close(); // Скрываем диалог прогресса
        auto result = watcher->result();
        watcher->deleteLater();
        if (result.second) {
            if (countElements(*result.first) > maxElements(*result.first)){
                QMessageBox::warning(widget, "Предупреждение", "Загружаемая матрица не является разряженной. Обработка невозможна.");
            } else {
